#!/usr/bin/env python3
"""
Demo script for real-time ETH order book and funding rate collection.
This script collects current ETH market data from Binance and outputs a JSON snippet.
"""
...
class RealtimeETHDemo:
    """
    Demo class for collecting real-time ETH orderbook and funding rate data.
    """
    ...
    def __init__(self):
        ...
        logger.info("RealtimeETHDemo initialized")
    
    async def collect_orderbook_data(self) -> Optional[Dict[str, Any]]:
        """
        Collect current ETH orderbook data from Binance.
    ...
    async def collect_funding_rate_data(self) -> Optional[Dict[str, Any]]:
        """
        Collect current ETH funding rate data from Binance.
    ...
    async def generate_eth_market_snapshot(self) -> Dict[str, Any]:
        """
        Generate a complete ETH market snapshot with orderbook and funding rate data.
        
        Returns:
            Dictionary with complete market data
        """
        logger.info("Generating ETH market snapshot...")
...
async def main():
    ...
    print("=" * 80)
    print("MTS Real-Time ETH Market Data Demo")
    print("=" * 80)
    print()
    
    # Generate filename first
    timestamp_str = datetime.now().strftime("%Y%m%d_%H%M%S")
    json_filename = f"eth_market_snapshot_{timestamp_str}.json"
    
    try:
        # Initialize demo
        demo = RealtimeETHDemo()
        
        # Generate market snapshot
        market_snapshot = await demo.generate_eth_market_snapshot()
        ...
        print("ETH Market Snapshot JSON:")
        print("-" * 40)
        print(json_output)
        print("-" * 40)
